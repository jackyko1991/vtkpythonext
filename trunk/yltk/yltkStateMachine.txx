/*=========================================================================
  ...
  Module:    $RCSfile: yltkStateMachine.txx,v $
  Language:  C++
  Date:      $Date: 2008-02-11 01:41:51 $
  Version:   $Revision: 1.49 $

  Copyright (c) ISC  Insight Software Consortium.  All rights reserved.
  See ylTKCopyright.txt or http://www.yltk.org/copyright.htm for details.
...
=========================================================================*/
#ifndef __yltkStateMachine_txx
#define __yltkStateMachine_txx

#include "yltkStateMachine.h"
#include "yltkEvents.h"
...
StateMachine< TClass >::StateMachine( TClass * owner )
{
  m_This = owner;
...
template<class TClass>
StateMachine< TClass >::~StateMachine()
{
...
template<class TClass>
void StateMachine< TClass >::AddInput( const InputType & input, const InputDescriptorType & descriptor )
{
...
template<class TClass>
void StateMachine< TClass >::AddState( const StateType & state, const StateDescriptorType & descriptor )
{
...
template<class TClass>
std::string StateMachine< TClass >::GetStateDescriptor( const StateIdentifierType & stateId )
{
...
template<class TClass>
std::string StateMachine< TClass >::GetInputDescriptor( const InputIdentifierType & inputId )
{
...
template<class TClass>
void StateMachine< TClass >::SelectInitialState( const StateType & initialState )
{

  if( m_ReadyToRun )
    {
    yltkLogMacroStatic( m_This, CRITICAL, "In class " 
...
template<class TClass>
void StateMachine< TClass >::SetReadyToRun()
{
  if( m_ReadyToRun )
    {
    yltkLogMacroStatic( m_This, CRITICAL, "In class " 
...
template<class TClass> void StateMachine< TClass >::PushInput( const InputType & input )
{
...
template<class TClass>
void StateMachine< TClass >::PushInputBoolean( bool condition, const InputType & inputIfTrue,
...
template<class TClass>
void StateMachine< TClass >::ProcessInputs()
{
...
template<class TClass>
void StateMachine< TClass >::ProcessInput( const InputIdentifierType & inputIdentifier )
{
  if( !m_ReadyToRun )
    {
    yltkLogMacroStatic( m_This, CRITICAL, "In class " 
  ...
  yltkLogMacroStatic( m_This, DEBUG, "State transition is being made : " 
    << m_This->GetNameOfClass() << " "
    << " PointerID " << m_This << " "
...
template<class TClass>
void StateMachine< TClass >::AddTransition( const StateType  & state,   
...
template<class TClass>
void StateMachine< TClass >::ExportDescription( OutputStreamType & ostr, bool skipLoops=false ) const
{
...
template<class TClass>
void StateMachine< TClass >::ExportDescriptionToLTS( OutputStreamType & ostr, bool skipLoops=false ) const
{
  // Export all the transitions between states
  int out_count = 0;
  int in_count = 0;
...
template<class TClass>
void StateMachine< TClass >::ExportDescriptionToSCXML( OutputStreamType & ostr, bool skipLoops ) const
{   
  // Export the descriptions of all states
  StatesConstIterator  stateId = m_States.begin();
...
/** Print Self function */
template<class TClass>
void StateMachine< TClass >::PrintSelf( std::ostream& os, yltk::Indent indent ) const
{
...
template<class TClass>
void StateMachine< TClass >::Print(std::ostream& os, yltk::Indent indent) const
{
